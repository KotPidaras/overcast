/obj/item/weapon/gun
	...
	var/damagelose = 0		 //1 óðîíà çà 1 òàéë = 0.33 åä
	var/distro = 0			 //Çàçîð ìåæäó äðîáüþ äëÿ äðîáîâèêîâ
				...
				if(!chambered.fire(target, user, params, distro, suppressed, zone_override, sprd, damagelose))
					...
					if(!jam)
						...
						if(!chambered.fire(target, user, params, distro, suppressed, zone_override, sprd, damagelose))
							shoot_with_empty_chamber(user)
							playsound(user, fire_sound, 50, 1)
							user << "<span class='warning'>Îðóæèå çàêëèíèëî. Åãî íåîáõîäèìî ïåðåçàð&#255;äèòü.</span>"
							firing_burst = 0
							return
						else
							user << "<span class='warning'>Îðóæèå çàêëèíèëî. Åãî íåîáõîäèìî ïåðåçàð&#255;äèòü.</span>"
...
/*
	if(durability < 1)
		...
		qdel(src)
		src = null
		user << "<span class='userdanger'>Îðóæèå ðàçîðâàëîñü ïð&#255;ìî ó âàñ â ðóêàõ!</span>"
	else
*/
	if(durability && !jam)
		...
		switch(percentage)
			...
			if(20 to 45)
				if(prob(10))
					user.drop_item()
					shake_camera(user, 4, 2)
					user << "<span class='userdanger'>Êàæåòñ&#255; ñàìîå âðåì&#255; ïî÷èíèòü ýòîò õëàì èëè âûáðîñèòü, ïîêà îí íå âûñòðåëèë òåáå â ëèöî.</span>"
					jam = 1
				else
					if(prob(20))
						jam = 1
